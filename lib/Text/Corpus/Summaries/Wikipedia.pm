...
have a well written lead section, or introduction; thus, the featured articles
...
C<af>:Afrikaans, C<ar>:Arabic, C<bg>:Bulgarian, C<ca>:Catalan, C<cs>:Czech, C<de>:German, C<el>:Greek, C<en>:English, C<eo>:Esperanto, C<es>:Spanish, C<eu>:Basque, C<fa>:Persian, C<fi>:Finnish, C<fr>:French, C<he>:Hebrew, C<hr>:Croatian, C<hu>:Hungarian, C<id>:Indonesian, C<it>:Italian, C<ja>:Japanese, C<ka>:Georgian, C<ko>:Korean, C<ml>:Malayalam, C<mr>:Marathi, C<ms>:Malay, C<nl>:Dutch, C<nn>:Norwegian, C<no>:Norwegian, C<pl>:Polish, C<pt>:Portuguese, C<ro>:Romanian, C<ru>:Russian, C<sh>:Serbocroatian, C<sk>:Slovak, C<sl>:Slovenian, C<sr>:Serbian, C<sv>:Swedish, C<th>:Thai, C<tr>:Turkish, C<vi>:Vietnamese, and C<zh>:Chinese.
The default is C<en>.
...
sub new
{
  ...
  $Self->{minSecondsBetweenFetches} = 8;
...
sub getParsedArticlePage # (htmlFile => '...')
{
  ...
  for (my $i = 1; $i < 10; $i++)
  {
    $xpathHeaders[$i-1] = $xpathBodyContent . '/h' . $i;
  }
  ...
  my $xpaths = join ('|', $xpathContentParagraphs, $xpathToc, $xpathHeaders, $xpathReferences);
  ...
  foreach my $node (@contentNodes)
  {
    ...
    # the strings returned should be utf8 encoded, if not, make thme so.
    ...
    for (my $i = 1; $i < 10; $i++)
    {
      if ($node->matches ($xpathHeaders[$i-1]))
      {
        $type = 'header' . $i;
        $headerFound = $i;
        push @lines, [$type, $value];
    ...
    # check is the node has the list of references.
    if ($node->matches ($xpathReferences))
    {
      $type = 'references';
      push @lines, [$type, $value];
      next;
...
sub _getFeaturedArticleLinkXpathPerLanguage
{
  ...
  my $xpath = '/html/body/div/div/div/ul/li/a/@href';
  ...
  # some wikipedias do not list their features articles on main featured articles page.
...
sub _setXpathForFeaturedArticles
{
  ...
  $featuredArticlesXpath{en} = '/html/body/div/div/table/tr[3]//a/@href';
  ...
  $featuredArticlesXpath{he} = '/html/body/div/div/table/tr[position()>4]/td//a/@href';
  ...
  $featuredArticlesXpath{nn} = '/html/body/div/div/div/div[2]/table/tr[2]/td/table[3]/tr[4]/td//ul//a/@href';
  ...
  $featuredArticlesXpath{th} = '/html/body/div/div/table[2]//a/@href';
...
sub _getFeaturedArticlesOfLanguageCodeFr
{
  ...
  while ($url)
  {
    ...
    my @listOfUrls = $htmlParser->findvalues ('/html/body/div/div/div/table[2]/tr/td/ul/li/a/@href');

    # log a warning if not urls extracted.
    unless (@listOfUrls)
    {
      my $logger = Log::Log4perl->get_logger();
      my $languageCode = $Self->{languageCode};
    ...
    my @potentialNextNodes = $htmlParser->findnodes ('/html/body/div/div/a');
    my $nextUrl;
    foreach my $node (@potentialNextNodes)
    {
      my $textOfNode = lc $node->findvalue ('.');
      if (index ($textOfNode, 'suivants') > -1)
      {
...
sub _getFeaturedArticlesOfLanguageCodeHe
{
  ...
  while ($url)
  {
    ...
    # get the list of featured article urls.
    my @listOfUrls = $htmlParser->findvalues ('/html/body/div/div/div/table/tr/td/ul/li/a/@href');
    ...
    # if there are a lot of repeats there are parsing problems.
    my @potentialNextNodes = $htmlParser->findnodes ('/html/body/div[3]/div/a');
    ...
    foreach my $node (@potentialNextNodes)
    {
      ...
      if ($textOfNode =~ /$nextPageLinkText/i)
      {
        $nextUrl = $node->findvalue ('@href');
        last;
      }
...
sub _setWikipediaLanguageCodes
{
  ...
  my $url = 'http://meta.wikimedia.org/wiki/List_of_Wikipedias_by_language_family';

  # build the full pathname of the file to save the page to.
  my $outputFile = File::Spec->catfile ($Self->{featuredDirectory}, 'List_of_Wikipedias_by_language_family.html');
  ...
  my $xpath = '/html/body/div/div/ol/li';
  my @listOfLanguageInfo = $htmlParser->findvalues ($xpath);
  ...
  {
    my $enLanguageName = $1 if ($languageInfo =~ /^([^ \(]+)/);
    my $languageCode = $1 if ($languageInfo =~ / ([^ ]+) [^ ]+ [\d\,]+ articles/);
...
sub _setWikipediaFeatureArticleInfo
{
  ...
  my $xpath = '/html/body/div/div/table/tr';
  ...
  foreach my $languageInfo (@listOfLanguageInfo)
  {
    my $url = lc $languageInfo->findvalue ('td[1]/a/@href');
    my $languageCode = $1 if ($url =~ /^http:\/\/([^\.]+)/);
...
<a href="http://fa.wikipedia.org/wiki/%D9%88%DB%8C%DA%A9%DB%8C%E2%80%8C%D9%BE%D8%AF%DB%8C%D8%A7:%D9%86%D9%88%D8%B4%D8%AA%D8%A7%D8%B1%D9%87%D8%A7%DB%8C_%D8%A8%D8%B1%DA%AF%D8%B2%DB%8C%D8%AF%D9%87">fa:Persian</a>,
<a href="http://fi.wikipedia.org/wiki/Wikipedia:Suositellut_sivut">fi:Finnish</a>,
...
<a href="http://hu.wikipedia.org/wiki/Wikip%C3%A9dia:Kiemelt_sz%C3%B3cikkek">hu:Hungarian</a>,
...
<a href="http://nn.wikipedia.org/wiki/Wikipedia:Gode_artiklar">nn:Norwegian</a>,
<a href="http://no.wikipedia.org/wiki/Wikipedia:Utmerkede_artikler">no:Norwegian</a>,
...
<a href="http://sh.wikipedia.org/wiki/Wikipedia:Izabrani_%C4%8Dlanci">sh:Serbocroatian</a>,
<a href="http://sk.wikipedia.org/wiki/Wikip%C3%A9dia:Najlep%C5%A1ie_%C4%8Dl%C3%A1nky">sk:Slovak</a>,
...
<a href="http://vi.wikipedia.org/wiki/Wikipedia:B%C3%A0i_vi%E1%BA%BFt_ch%E1%BB%8Dn_l%E1%BB%8Dc">vi:Vietnamese</a>, and
<a href="http://zh.wikipedia.org/wiki/Wikipedia:%E7%89%B9%E8%89%B2%E6%9D%A1%E7%9B%AE">zh:Chinese</a>.</p>
